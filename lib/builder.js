...
(function () {
  ...
  exports.init = function (grunt) {
    ...
    exports.build = function (options, contentCb) {
      ...
      if (semver.gte(lodashTarget.VERSION, '0.7.0')) {
        // check for debug or minified output
        if (options.debug && options.minify) {
          mappedConfig.push('--debug --minify');
        } else if (options.debug && !options.minify) {
          mappedConfig.push('--debug');
        } else if (!options.debug && options.minify) {
          mappedConfig.push('--minify');
        }
        ...
        lodashBuilder(args, function (output) {
          ...
          output = (_.isObject(output)) ? output.source : output;
    ...
    exports.parseArgs = function (config, mappedConfig) {
      ...
      _.each(config, function (item, key) {
        if (key !== 'src' && key !== 'dest' && key !== 'alias' && key !== 'modifier' && key !== 'debug' && key !== 'minify') {
